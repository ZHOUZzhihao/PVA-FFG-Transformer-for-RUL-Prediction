...
class Gating(nn.Module):
    def __init__(self, m, d_model, seq_len):  # #14，64，30，input size (batch*30*14)
        super().__init__()
        self.m = m
        self.seq_len = seq_len
        # Self-attention layer
        self.self_attention = nn.MultiheadAttention(m, num_heads=1)
        ...
        self.cnn_layers = nn.Sequential(
            nn.Conv1d(seq_len, seq_len, kernel_size=3, stride=1, padding=1),#30，30
            nn.Conv1d(seq_len, seq_len, kernel_size=3, stride=1, padding=1),#30，30
        )
    ...
    def forward(self, x):
        ...
        x_self_attn, _ = self.self_attention(x, x, x)
        ...
        hh_i = torch.mul(h_i, u_i) + torch.mul(x_i, 1-u_i)
        # hh_i = x
        intermediate_features = hh_i.detach().cpu().numpy()  # 将特征移至 CPU 并转换为 NumPy 数组


        return torch.matmul(hh_i,self.W_e) + self.b_e,intermediate_features # (the final output is 1,1,1,128 as the encoder has size of 128.)
...
class AUM_Transformer(nn.Module):
    def __init__(self, seq_size=30, patch_size=3, in_chans=14,
        ...
        self.seq_tf = nn.Sequential(
            nn.Linear(in_features=seq_size*embed_dim, out_features=50),#30*64
        ...
        self.gating = Gating(in_chans, embed_dim, seq_size)#14，64，30
    ...
    def forward(self, seq):
            gcu_out, intermediate_features= self.gating(seq)
            intermediate_features = intermediate_features#.detach().cpu().numpy()  # 将特征移至 CPU 并转换为 NumPy 数组
